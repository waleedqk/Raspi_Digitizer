...
run_experiment()
{
    ...
    case "$LOAD_VAL" in
        ...
        2)  echo  "51kb file to be created"
            $(sudo -u ${SUDO_USER} dd if=/dev/urandom of=${PAYLOAD_DIR}/payload51kb.dat bs=51k count=1)
            ;;
        3)  echo  "1MB file to be created"
            $(sudo -u ${SUDO_USER} dd if=/dev/urandom of=${PAYLOAD_DIR}/payload1MB.dat bs=1M count=1)
            ;;
        4)  echo  "10MB file to be created"
            $(sudo -u ${SUDO_USER} dd if=/dev/urandom of=${PAYLOAD_DIR}/payload10MB.dat bs=1M count=10)
            ;;
        0|*)  
    ...
    case "$CONFIG_VAL" in
        1)  echo  "Copy payload file to single sender"
            $(scp -r -i ~/.ssh/id_ras_pi ${PAYLOAD_DIR}/*.dat pi@raspiDigitizer2.local:Downloads/payload)
            ;;
